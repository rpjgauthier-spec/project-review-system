...
This stage includes minimalism, but it is not limited to removal. It may retain, defer, narrow, simplify, merge, split, index, generate, restructure, or remove elements when the resulting design is materially better and no required protection or consumer is lost.
...
7. Is deferral, narrowing, merging, splitting, indexing, or generation safer than removal?
...
Do not create a permanent document, tracker, checklist, status, role, recurring review, evaluation obligation, generated view, or update duty unless an existing mechanism cannot reliably serve the material consumer.
...
A stage is `Complete` when no material unnecessary burden or materially inferior structure remains within the reviewed scope under the current state and known triggers. It may be `Conditional` when named external facts determine whether a dormant structure activates, remains deferred, or should be reorganized.
