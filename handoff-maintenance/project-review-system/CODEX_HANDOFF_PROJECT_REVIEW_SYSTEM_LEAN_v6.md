...
1. Until the governing PRS controls for this recovery are established, operate **Diagnostic-only**: perform only non-destructive discovery needed to locate the repository and governing context, and do not perform semantic review or any action requiring broader authority.
...
2. Establish the current governing implementation context.
   - Start from `implementation-phase-2-controller-core` as a locator, not as authority.
...
3. From that legitimate recovery context, read the minimum current PRS authority needed to determine review mode, status machinery, and current-state discovery:
   ...
   Do not treat PRS controls from the initial checkout as governing merely because they were encountered first; bind the controls relied upon for recovery to the legitimate governing context.

4. Establish the current PRS review mode, scope, allowed actions, and review depth under those controls.
...
9. Before the first semantic interpretation of recovery state, run the current PRS Adaptive Execution preflight/gate required for the established mode, scope, depth, revision, and target state. Mechanical discovery may precede this boundary; semantic judgment may not.

10. Reproduce the historical deterministic failure or establish its current governed disposition/replacement failure from current repository state.

11. Before relying on existing review credit, verify with current PRS controls that it is bound to the current review revision and target state.

12. When historical credit is disputed, reconstruct both:
...
13. Continue through the next valid governed recovery action unless a stop condition below is reached.
